...
def test_protocols_convergence():
    ...
    for rname, router in tgen.routers().items():
        ...
        if result is not None:
            ...
            logger.info("{}: show run:\n{}".format(rname, router.vtysh_cmd("show run")))
            logger.info(
                "{}: ip -d link show {}-gre0:\n{}".format(
                    rname, rname, router.run("ip -d link show {}-gre0".format(rname))
                )
            )
            logger.info(
                "{}: show interface {}-gre0:\n{}".format(
                    rname,
                    rname,
                    router.vtysh_cmd("show interface {}-gre0".format(rname)),
                )
            )
            logger.info(
                "{}: ip route show proto 191:\n{}".format(
            ...
            logger.info(
                "{}: nhrpd.log (last 80 lines):\n{}".format(
                    rname, router.run("tail -80 nhrpd.log 2>/dev/null || echo 'no log'")
                )
            )
            hub = tgen.gears["r2"]
            logger.info(
                "r2: nhrpd.log (last 40 lines):\n{}".format(
                    hub.run("tail -40 nhrpd.log 2>/dev/null || echo 'no log'")
                )
            )
            logger.info(
                "r2: show ip nhrp cache:\n{}".format(
...
def test_encap_routes():
    ...
    def _check_encap_route(router, dst, nbma):
        ...
        if "encap ip6" not in output or nbma not in output:
            return "{}: missing encap ip6 for {} -> {}".format(router.name, dst, nbma)
        return None
