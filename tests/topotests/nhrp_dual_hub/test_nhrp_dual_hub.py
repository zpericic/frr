...
def ping_test(source_router, target_ip, count=1000, description=""):
    tgen = get_topogen()
    logger.info("Ping %s -> %s %s", source_router.name, target_ip, description)
...
def test_protocols_convergence():
    ...
    for rname in ["hub1", "hub2", "nhc1", "nhc2"]:
        ...
        if result is not None:
            log = router.cmd("tail -n 100 nhrpd.log || true")
...
def test_no_hop_count_errors():
    ...
    for rname in ["hub1", "hub2", "nhc1", "nhc2"]:
        router = tgen.gears[rname]
        log = router.cmd("grep -c 'hop count exceeded' nhrpd.log || true").strip()
...
def test_purge_on_clear():
    ...
    def check_purge_log(router):
        log = router.cmd("grep -c 'Purge-Request' nhrpd.log || true").strip()
        count = int(log) if log.isdigit() else 0
        if count > 0:
            return None
    ...
    _, result = topotest.run_and_expect(
        partial(check_purge_log, hub1), None, count=20, wait=0.5
    )
