...
int os_gre_socket(void)
{
	...
	if (setsockopt(fd, IPPROTO_IP, IP_PKTINFO, &on, sizeof(on)) < 0) {
		zlog_err("%s: IP_PKTINFO: %s", __func__, safe_strerror(errno));
		close(fd);
		return -1;
	}
...
int os_gre6_socket(void)
{
	...
	if (setsockopt(fd, IPPROTO_IPV6, IPV6_RECVPKTINFO, &on,
		       sizeof(on)) < 0) {
		zlog_err("%s: IPV6_RECVPKTINFO: %s", __func__,
			 safe_strerror(errno));
		close(fd);
		return -1;
	}
		   ...
		   uint32_t grekey)
{
	...
	debugf(NHRP_DEBUG_KERNEL, "%s: %pSU -> %pSU key %u len %zu",
	       __func__, src_nbma, dst_nbma, grekey, len);
	...
	if (ret < 0) {
		debugf(NHRP_DEBUG_KERNEL, "%s: failed: %s", __func__,
		       safe_strerror(errno));
		return -errno;
	}
